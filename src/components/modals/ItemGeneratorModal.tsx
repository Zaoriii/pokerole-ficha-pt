...
export function ItemGeneratorModal({ onClose }: ItemGeneratorModalProps) {
    ...
    const handleGenerate = async () => {
        const index = await loadLocalDataset();
        if (!index) {
            if (OBR.isAvailable) OBR.notification.show('⚠️ Dataset local não encontrado.', 'ERROR');
            return;
        }
        ...
        if (masterPool.length === 0) {
            if (OBR.isAvailable) OBR.notification.show('⚠️ Nenhum item corresponde a estes filtros!', 'WARNING');
            return;
        }
        ...
        try {
            ...
            if (OBR.isAvailable) OBR.notification.show('⚠️ Erro ao gerar item.', 'ERROR');
        } finally {
            ...
            <div className="item-generator-modal__overlay">
                <div className="item-generator-modal__content">
                    <div className="item-generator-modal__header">
                        <h3 className="item-generator-modal__title">🎁 Gerador de Itens Aleatórios</h3>
                        <button onClick={onClose} className="item-generator-modal__close-btn" title="Fechar">
                            X
                        </button>
                    </div>
                    <p className="item-generator-modal__desc">Selecione uma ou mais categorias para montar seu pool de itens.</p>
                    ...
                    <div className="item-generator-modal__rarity-container">
                        <div className="item-generator-modal__rarity-header">✨ Filtros de Raridade de Drop</div>
                        <div className="item-generator-modal__rarity-grid">
                            ...
                            <div
                                ...
                                title="Se marcado, Poções Comuns e Masterballs Lendárias terão exatamente a mesma probabilidade de aparecer!"
                            >
                                <label className="item-generator-modal__checkbox-label">
                                    ...
                                    />
                                    Ignorar Ponderação (Chances Iguais)
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="item-generator-modal__filters-container">
                        <div className="item-generator-modal__filter-group">
                            <div className="item-generator-modal__filter-group-title">🎒 Itens Base</div>
                            {basePockets.length === 0 && <div className="item-generator-modal__desc">Carregando...</div>}
                        ...
                        <div className="item-generator-modal__filter-group">
                            <div className="item-generator-modal__filter-group-title">🛠️ Itens Personalizados</div>
                            {customPockets.length === 0 && (
                                <div className="item-generator-modal__desc">Nenhum Item Personalizado disponível.</div>
                            )}
