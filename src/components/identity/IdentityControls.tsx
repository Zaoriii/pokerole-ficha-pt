...
export function IdentityControls({
    ...
    const handleExport = async () => {
        ...
        if (!state.tokenId || !OBR.isAvailable || !isObrReady) {
            if (OBR.isAvailable && isObrReady) OBR.notification.show('Selecione um token para exportar.', 'WARNING');
            return;
        }
    ...
    const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (fileEvent) => {
            try {
                ...
                if (OBR.isAvailable && isObrReady) OBR.notification.show('Arquivo JSON inválido.', 'ERROR');
            }
    ...
    const confirmImport = () => {
        ...
        try {
            ...
            if (OBR.isAvailable && isObrReady) OBR.notification.show('Falha ao importar dados.', 'ERROR');
        } finally {
            setImportData(null);
        }
    };
            ...
            <div className="identity-header__actions">
                ...
                <div style={{ display: 'flex', gap: '0px', flex: '1 1 25%', minWidth: '120px', maxWidth: '100%' }}>
                    ...
                    <button
                        ...
                        title="Configurações de Iniciativa"
                    >
                        ⚙️
                    </button>
                </div>
                ...
                <button
                    ...
                    title="Exportar Personagem (Baixar JSON)"
                >
                    💾
                </button>
                <button
                    ...
                    className="action-button action-button--dark identity-header__btn--small"
                    title="Importar Personagem (Enviar JSON)"
                >
                    📂
                </button>
                <input
            ...
            {importData && (
                <div className="identity-header__modal-overlay">
                    <div className="identity-header__modal-content">
                        <h3 className="identity-header__modal-title">⚠️ Confirmar Importação</h3>
                        <p className="identity-header__modal-text">
                            Importar dados do personagem? Isso substituirá completamente o token atual.
                        </p>
